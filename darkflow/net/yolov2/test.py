...
def postprocess(self, net_out, im, save = True):
	...
	for row in range(H):
		for col in range(W):
			for b in range(B):
				...
				#bx.probs = _softmax(classes) 
				bx.probs *= bx.probs > (threshold/10)
				boxes.append(bx)
	...
	for b in boxes:
		...
		if max_prob > threshold:
			...
			thick = int((h+w)/300)
			mess = '{}'.format(label)
			mess_frame = '{} {:.2%}'.format(label, max_prob)
			if self.FLAGS.json:
				line = 	('{"label":"%s",'
					'"confedence":%f,'
					'"ALB":%f,'
			...
			cv2.putText(imgcv, mess_frame, (left, top - 12),
				0, 1.75e-3 * h, colors[max_indx],thick//3)
